...
> ⚠️ **정식 루프 밖에서 빠르게 전/후 스크린샷을 픽셀 비교할 때(예: 패딩 값 하나만 급히 검증)**, 풀스크린
> 캡처는 앱 캔버스 아래 **OS 내비게이션 바(3버튼 모드의 ⋮○< 아이콘 행)까지 그대로 포함한다.** "화면 맨
> 아래부터 스캔해서 첫 어두운 픽셀을 찾는다" 류의 단순 비교 스크립트는 콘텐츠가 바뀌어도 그 자리에 고정된
> 내비바 아이콘을 매번 잡아 **delta=0을 오보고**한다(실측: 패딩을 24→40, 다시 40→64로 두 번 바꿨는데
> "맨 아래 어두운 행" 스캔은 둘 다 동일한 y좌표를 반환했다 — 실제로는 카드 하단 흰 배경이 시작되는 지점을
> 찾아야 했다). 전/후 비교는 (a) 내비바 영역 높이만큼 미리 크롭해서 제외하거나, (b) 카드의 알려진 배경색
> (흰색 등)으로 전환되는 지점을 찾거나, (c) 아예 크롭 이미지를 직접 눈으로 대조한다 — "어두운 픽셀 스캔"
> 하나만으로 판정하지 않는다.
