...
- **유형 A:** SVG → `SvgPicture.asset('assets/icons/ic_xxx.svg')`(`flutter_svg` 의존 필요 — pubspec `dependencies`에 없으면 추가), PNG → `Image.asset('assets/images/img_xxx.png')`. PNG 경로는 **main asset 경로**(`2.0x/` 미포함)로 참조한다 — 실제 파일이 `assets/images/2.0x/`에만 있어도 `AssetImage`가 device pixel ratio에 맞춰 해소하고(공식 "Resolution-aware image assets"), width/height 미지정 시 main 기준 논리 크기로 렌더한다. 더 높은 배율이 필요하면 Figma REST `scale=3`으로 재추출해 `3.0x/`에 둔다.
...
**pubspec 등록 필수:** `convert_assets.sh`는 에셋을 복사·배치만 하고 pubspec은 수정하지 않는다(YAML 자동 수정은 들여쓰기 파손 위험). 스크립트가 미선언 WARN을 찍으면 `flutter:`→`assets:` 아래에 `assets/icons/`·`assets/images/` **디렉터리 항목**을 Edit로 추가한다. 디렉터리 선언은 직속 파일만 포함하되 resolution variant(`2.0x/` 등)는 **예외적으로 자동 번들**되므로 `2.0x/`를 별도로 선언하지 않는다. `flutter_svg`를 처음 쓰면 `dependencies`에도 추가한다.
